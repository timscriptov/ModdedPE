...
class NModExtractor {
    ...
    public NModExtractor(Context context) {
        mContext = context;
    }

    public PackagedNMod archiveFromInstalledPackage(String packageName) throws ExtractFailedException {
        try {
    ...
    public ZippedNMod archiveFromZipped(String path) throws ExtractFailedException {
                ...
                try (ZipFile zipFile = new ZipFile(path);
                    ...
                    while ((entry = zipInput.getNextEntry()) != null) {
                        if (!entry.isDirectory() && !(entry.getName().equals(NMod.MANIFEST_NAME) || entry.getName().endsWith(File.separator + NMod.MANIFEST_NAME))) {
                            ...
                            try (InputStream from = zipFile.getInputStream(entry)) {
                                int byteRead;
    ...
    private File copyCachedNModToData(File cachedNModFile, String packageName) throws ExtractFailedException {
        try {
            ...
            try (FileOutputStream finalFileOutput = new FileOutputStream(finalFile);
    ...
    private NMod.NModInfo archiveInfoFromZipped(File filePath) throws ExtractFailedException {
        try (ZipFile zipFile = new ZipFile(filePath)) {
            ...
            try (InputStream input = zipFile.getInputStream(manifest)) {
                int byteRead;
                byte[] buffer = new byte[1024];
                StringBuilder tmp = new StringBuilder();
                while ((byteRead = input.read(buffer)) > 0) {
                    tmp.append(new String(buffer, 0, byteRead));
                }
                return new Gson().fromJson(tmp.toString(), NMod.NModInfo.class);
